...
async def main():
    ipv6 = ip_utils.get_ip_if_changed(6, False)
    if ipv6 is None:
        return
    ...
    async with router_automation_async.RouterAutomationRunnerAsync(headless=True, screenshot=True) as runner:
        # 执行登录
        await runner.login()
        # 导航到端口映射页面
